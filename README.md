...
- **🤖 AI 가성비** — 최근 3개월 실거래를 분석해 **같은 동·같은 평형대 대비 저평가 단지**를 랭킹.
  저평가45%·연식20%·유동성20%·추세15% 가중, 근거(할인율·거래수·추세) 전부 공개.
- **📍 직주근접** — 근무지(회사명/주소 지오코딩)를 저장하고 **반경 3~15km 내 가성비 단지**를
  실거리와 함께 탐색.
...
- [x] 정책 브리핑 최신 소식 자동 수집(국토부·금융위·korea.kr RSS + 해외IP 시 구글뉴스 폴백)
- [x] 단지 상세: 카드 클릭 → 12개월 시세 추이 차트 + 최근 거래
- [x] 관심 단지 즐겨찾기(저장가 대비 최신가 변동)
- [x] 새 청약 공고 알림(탭 배지 + NEW 하이라이트)
- [x] 지도 가격 히트맵 모드(구별 중위가 그라데이션)
